...
`scripts/bench_backfill.sh` re-measures *historical* commits with the current bench sources: one git worktree per commit, this tree's `bench/google/` copied in, recorded against the commit's own sha and commit date. `bench_pipeline.sh` cannot do this — it always records HEAD at wall-clock now. Use it after changing a benchmark, because records made by two different versions of a benchmark are not comparable and will otherwise put a fake cliff in the trend chart.

How far back it reaches depends on which benchmark. `bench_matching_engine.cpp` needs `Symbol` (`a925d17`), `bench_ring.cpp` needs `net_protocol` (`560595f`) and `bench_flash1_replay.cpp` needs both, so none of those can be built against an older engine. `bench_orderbook.cpp` can: the engine API it exercises is unchanged back to `612785f`, and only book *construction* differs, which `BenchSupport.hpp` absorbs with a `__has_include("types/Symbol.hpp")` guard around `makeOrderBook()`. So a tree with no `CMakePresets.json` takes the script's **legacy path** — flat `bench/`, `bench_matching_engine.cpp` blanked, its own untouched `bench/CMakeLists.txt` (which already pins googlebenchmark v1.9.5, links `benchmark_main` and sets `-O3 -march=native`, identical to HEAD's), built with a plain `-DCMAKE_BUILD_TYPE=Release` that is sanitizer-free because `debug_options` gates every sanitizer behind `$<$<CONFIG:Debug>>`. Those commits get a reduced 9-benchmark OrderBook suite covering every curated `HISTORY_LATENCY_NS` metric plus `BM_MixedWorkload_SteadyState`; only `BM_Engine_MultiBook_SteadyState` is missing from those trend points. flash1 runs there through the tree's own `run_flash1.sh`, which needs no presets.

The guard is legitimate rather than a fudge because it touches only fixture construction, never a timed region — the operations measured stay identical, which is the rule the whole backfill design exists to protect. Note `restingBuyIds` returns worst-price-first on pre-`586ecc6` trees; both callers use it as an unordered pool, and the two `_AtDepth` benchmarks address levels by *price*, which is exactly what makes them comparable across the reversal.
