...
`bench/CMakeLists.txt` is a pure dispatcher over two peer suites, each owning its own `CMakeLists.txt`: `bench/google/` (the Google Benchmark microbenchmarks — target `exchange_bench`, which also pins the googlebenchmark FetchContent dependency) and `bench/flash1/` (the external conformance harness adapter). Benchmark results and plots land in the gitignored `bench/results/`, one directory per measured commit.
...
`scripts/bench_pipeline.sh` runs the Google Benchmark suite (Release) plus one flash1 `perf` rep per scenario, writes a timestamped + git-SHA-tagged record JSON to gitignored `bench/results/<shortsha>/`, and renders PNGs. Flags: `--full` (10 flash1 reps/scenario — challenge-style scoring), `--skip-flash1`, `--skip-hist`, `--plot-only`. Plotting needs `uv`; Python deps are declared inline in `scripts/plot_bench.py` (PEP 723). The flash1 baseline used for reference lines is hardcoded in `plot_bench.py` (`BASELINE_MPS`) — keep it in sync with the numbers below.

**Each measured commit owns a directory.** `bench/results/<shortsha>/` holds that commit's record *and* the snapshot plots rendered from it (`flash1.png`, `gb.png`, `flash1_latency.png`), so one commit's data can be analysed on its own; only the cross-commit trend charts (`flash1_history.png`, `gb_history.png`) live at the root in `bench/results/plots/`. A directory counts as a commit's iff its name is exactly seven hex characters — that shape test, not an allow-list, is what keeps `plots/`, `net/` and `archive_broken_suite/` out of the history walk while they sit as siblings. A commit measured more than once keeps every record, and the newest drives its plots. `--plot-only` re-renders every commit's snapshots plus the trend. Plots reach git only via `scripts/publish_bench_plots.sh`, which copies the README's PNGs into tracked `docs/bench/<shortsha>/` and prints the matching markdown through `scripts/bench_summary.py`.
...
`scripts/bench_backfill.sh` re-measures *historical* commits with the current bench sources: one git worktree per commit, this tree's `bench/google/` copied in, recorded against the commit's own sha and commit date. `bench_pipeline.sh` cannot do this — it always records HEAD at wall-clock now. Use it after changing a benchmark, because records made by two different versions of a benchmark are not comparable and will otherwise put a fake cliff in the trend chart. Flags: `--flash1-reps N`, `--skip-flash1`, `--skip-hist`, `--hist-only`.

**`--hist-only`** runs only the per-op latency pass and folds it into the record each commit already has, through `plot_bench.py --update-hist` (loads that commit's newest record, fills `flash1_latency`, rewrites at the same path, re-renders). It exists because that measurement is expensive and orthogonal to throughput: merging is safe precisely because the key is additive, so a set of commits measured together keeps its one-sitting throughput numbers — and anything quoting them stays true — rather than being re-measured on a different day. Normal mode runs the same pass inline, so a fresh backfill is complete in one go.

How far back it reaches depends on which benchmark. `bench_matching_engine.cpp` needs `Symbol` (`a925d17`) and `bench_ring.cpp` needs `net_protocol` (`560595f`), so neither can be built against an older engine. `bench_orderbook.cpp` and `bench_flash1_replay.cpp` can: the engine API they exercise is unchanged back to `612785f`, and only book *construction* differs, which `BenchSupport.hpp` absorbs with a `__has_include("types/Symbol.hpp")` guard around `makeOrderBook()`. So a tree with no `CMakePresets.json` takes the script's **legacy path** — flat `bench/` holding only the sources that compile plus everything the replay needs (`Flash1Bench.hpp`, `LatencyHistogram.hpp`, `TscTimer.hpp`, and `Flash1Workload.{hpp,cpp}` from `bench/net/`, which is standard-library only), a **generated** `bench/CMakeLists.txt`, and a plain `-DCMAKE_BUILD_TYPE=Release` build that is sanitizer-free because `debug_options` gates every sanitizer behind `$<$<CONFIG:Debug>>`. The historical `CMakeLists.txt` has to be replaced rather than reused because it names `bench_matching_engine.cpp` and links `benchmark::benchmark_main`, while `--hist-json` is parsed by our own `bench_main.cpp`; it is generated from HEAD's settings (same googlebenchmark v1.9.5 pin, same `-O3 -march=native`) so it cannot drift from the modern path. Those commits get a reduced 9-benchmark OrderBook suite covering every curated `HISTORY_LATENCY_NS` metric plus `BM_MixedWorkload_SteadyState`, *and* the full latency distribution; only `BM_Engine_MultiBook_SteadyState` is missing from those trend points. flash1 runs there through the tree's own `run_flash1.sh`, which needs no presets.

The guard is legitimate rather than a fudge because it touches only fixture construction, never a timed region — the operations measured stay identical, which is the rule the whole backfill design exists to protect. The replay benchmark qualifies for a second reason: `replayStream` uses only `addOrder`, `removeOrder` and `Fill`, never `buys()`/`sells()`, so it is layout-independent in a way the flash1 *adapter* is not. Note `restingBuyIds` returns worst-price-first on pre-`586ecc6` trees; both callers use it as an unordered pool, and the two `_AtDepth` benchmarks address levels by *price*, which is exactly what makes them comparable across the reversal.
...
- Registered with **`->Iterations(1)`**, which short-circuits Google Benchmark's ramp so one repetition is exactly one stream replay. Without it, samples from the discarded warm-up trials blend into the published histogram. Beware that this also **suffixes the registered names** with `/iterations:1`, so an end-anchored filter like `'^BM_Flash1_Replay/normal$'` silently matches nothing; the unanchored `'^BM_Flash1_'` both pipelines use is unaffected.
...
It is available for *historical* commits too, via `bench_backfill.sh --hist-only` — `replayStream` touches no ordering-dependent API, so one identical instrument runs at every point in the range.
...
`net_workload_bench` (`bench/net/`, Release only, `cmake --build --preset net-bench`) replays the **flash1 order stream** through the networking layer, so the network numbers sit on the same axis as the engine numbers — same five volatility regimes, same messages, no second synthetic workload to keep honest. `scripts/net_bench_pipeline.sh` is the `bench_pipeline.sh` equivalent: run, record to `bench/results/net/<shortsha>/run_<ts>_<sha>.json`, plot beside it — the same per-commit layout, with only `net_history.png` at `bench/results/net/plots/`.
